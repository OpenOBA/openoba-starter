  ...
  <el-config-provider :z-index="3000">
    ...
    <div v-else class="app-shell">
      <header class="top-nav" :class="{ 'chat-mode': isChatActive }">
        ...
        <div class="nav-right">
          ...
          <div class="status-block" title="系统运行中">
            <svg class="status-dot" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <use href="/lucide-sprite.svg#circle-check" />
            </svg>
            <span class="status-label">运行中</span>
          </div>

          <span v-if="isChatActive" class="menu-toggle" @click="menuOpen = !menuOpen">&#9776;</span>
          <el-button text size="small" style="color: #909399" @click="handleLogout">退出</el-button>

          <!-- 版本/升级信息：最右侧，低频低调 -->
          <el-popover placement="bottom-end" :width="280" trigger="click" @show="checkUpdate">
            <template #reference>
              <span
                class="version-hint"
                :class="{ 'has-update': hasUpdate }"
                :title="hasUpdate ? `有新版本 v${latestVersion}` : `v${currentVersion}`"
              >
            ...
            <div class="version-popover">
              ...
              <div v-if="hasUpdate" class="vp-row">
      ...
      <nav v-if="menuOpen && isChatActive" class="sub-nav chat-mode">
        <router-link to="/chat" class="sub-item" :class="{ active: $route.path === '/chat' }" @click="menuOpen = false"
          >对话</router-link
        >
        <router-link
          to="/chat/agents"
          class="sub-item"
          :class="{ active: $route.path === '/chat/agents' }"
        ...
        <router-link
          to="/chat/erdl"
          class="sub-item"
          :class="{ active: $route.path === '/chat/erdl' }"
          @click="menuOpen = false"
          >ERDL</router-link
        >
        <router-link
          to="/chat/playground"
          class="sub-item"
          :class="{ active: $route.path === '/chat/playground' }"
          @click="menuOpen = false"
          >实验</router-link
        >
        <router-link
          to="/chat/skills"
          class="sub-item"
          :class="{ active: $route.path === '/chat/skills' }"
          @click="menuOpen = false"
          >技能</router-link
        >
        <router-link
          to="/chat/deployment"
          class="sub-item"
          :class="{ active: $route.path === '/chat/deployment' }"
          @click="menuOpen = false"
          >交付</router-link
        >
        <router-link
...
// 版本 / 升级信息
// ============================================
const currentVersion = ref('1.6.0-alpha')
...
.top-nav.chat-mode .nav-brand {
  position: absolute;
  left: 208px;
}
.top-nav.chat-mode .nav-links {
  padding-left: 200px;
}
.top-nav.chat-mode .nav-right {
  right: 20px;
}
...
.version-hint.has-update .version-hint-text {
  color: #e6a23c;
}
.update-dot {
  width: 5px;
