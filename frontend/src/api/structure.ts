...
export const getStructureDetail = (id: string): Promise<StructureStandard> =>
  request.get(`/structures/${id}`)
...
export const deleteStructure = (id: string): Promise<void> =>
...
export const addCompatibility = (data: { structureStandardCode: string; productSkuId: string; compatibilityLevel: string; notes?: string }): Promise<Record<string, unknown>> =>
  request.post('/structures/compatibilities', data)

export const deleteCompatibility = (id: string): Promise<void> =>
  request.delete(`/structures/compatibilities/${id}`)

// 字典查询
export const getDictShapes = (): Promise<DictItem[]> =>
  request.get('/dict/structure_shape')

export const getDictSeries = (): Promise<DictItem[]> =>
  request.get('/dict/structure_series')
