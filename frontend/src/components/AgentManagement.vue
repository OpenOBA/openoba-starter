<template>
  <div class="agent-management">
    ...
    <el-table v-loading="loading" :data="filteredAgents" stripe style="margin-top: 12px">
      ...
      <el-table-column label="操作" width="200" fixed="right">
        <template #default="{ row }">
          ...
          <template v-if="row.agentType !== 'main'">
            <el-button v-if="row.status !== 'suspended'" type="warning" link size="small" @click="handleSuspend(row)"
              >挂起</el-button
            >
            <el-button v-else type="success" link size="small" @click="handleActivate(row)">恢复</el-button>
            <el-button v-if="row.status !== 'active'" type="danger" link size="small" @click="handleDelete(row)"
              >删除</el-button
            >
          </template>
        </template>
      ...
      <template v-if="editing">
        ...
        <el-form :model="editForm" label-width="90px" style="margin-top: 16px">
          ...
          <el-form-item label="安全等级">
            <el-select
              v-model="editForm.securityClearance"
              size="small"
              style="width: 100%"
              :disabled="editMode === 'view'"
            >
