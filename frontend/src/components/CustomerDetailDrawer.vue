...
-->
<template>
  <el-drawer v-model="internalVisible" :title="'客户详情 — ' + (detail?.contactName || '')" size="900px" destroy-on-close @close="emit('close')">
      ...
      <el-tabs v-if="detail.customerType === 'retail'" v-model="activeTab" @tab-click="onTabClick">
        ...
        <el-tab-pane label="验光处方" name="prescription">
          <el-table :data="prescriptions" size="small" border style="margin-bottom: 12px">
            ...
            <el-table-column label="右眼(OD)" width="200">
              <template #default="{ row }">S:{{ row.odSphere || '—' }} C:{{ row.odCylinder || '—' }} A:{{ row.odAxis || '—' }}<span v-if="row.odAdd"> ADD:{{ row.odAdd }}</span></template>
            </el-table-column>
            <el-table-column label="左眼(OS)" width="200">
              <template #default="{ row }">S:{{ row.osSphere || '—' }} C:{{ row.osCylinder || '—' }} A:{{ row.osAxis || '—' }}<span v-if="row.osAdd"> ADD:{{ row.osAdd }}</span></template>
            </el-table-column>
            ...
            <el-table-column label="过期日期" width="110">
              <template #default="{ row }">
                <el-tag :type="isExpired(row.expireDate) ? 'danger' : 'success'" size="small">{{ row.expireDate || '长期有效' }}</el-tag>
              </template>
            </el-table-column>
        ...
        <el-tab-pane label="在用结构标准" name="structure">
          ...
          <el-table :data="customerLenses" size="small" border style="margin-bottom: 12px">
            ...
            <el-table-column label="状态" width="80">
              <template #default="{ row }"><el-tag :type="row.status === 'active' ? 'success' : 'info'" size="small">{{ row.status === 'active' ? '在用' : '停用' }}</el-tag></template>
            </el-table-column>
            <el-table-column label="关联处方" width="100">
              <template #default="{ row }"><span v-if="row.prescriptionId">已关联</span><span v-else style="color: #999">未关联</span></template>
            </el-table-column>
            <el-table-column label="购买日期" width="110"><template #default="{ row }">{{ row.purchaseDate || '—' }}</template></el-table-column>
            <el-table-column label="来源订单" width="140"><template #default="{ row }">{{ row.orderId || '—' }}</template></el-table-column>
            <el-table-column label="创建时间" width="160"><template #default="{ row }">{{ row.createdAt }}</template></el-table-column>
            <el-table-column label="操作" width="80" fixed="right">
              <template #default="{ row }">
        ...
        <el-tab-pane label="消费记录" name="orders">
          <el-table :data="customerOrders" size="small" border v-loading="ordersLoading" style="margin-bottom: 12px">
            ...
            <el-table-column label="状态" width="80">
              <template #default="{ row }"><el-tag :type="orderStatusTag(row.status)" size="small">{{ orderStatusLabel(row.status) }}</el-tag></template>
            </el-table-column>
            <el-table-column label="商品数" width="60" align="center"><template #default="{ row }">{{ row.items?.length || 0 }}</template></el-table-column>
            <el-table-column label="订单金额" width="100" align="right"><template #default="{ row }">¥{{ Number(row.totalAmount || 0).toFixed(2) }}</template></el-table-column>
            <el-table-column label="创建时间" width="160"><template #default="{ row }">{{ row.createdAt }}</template></el-table-column>
          </el-table>
          <el-pagination v-if="orderTotal > 0" v-model:current-page="orderPage" :page-size="10" :total="orderTotal" layout="prev, pager, next" small @current-change="loadCustomerOrders" style="justify-content: flex-end" />
        ...
        <el-tab-pane label="官网账户" name="account">
          <el-empty v-if="!websiteAccount" description="该客户尚未注册官网账户" :image-size="80">
            <el-button type="primary" size="small" @click="handleRegisterAccount" :loading="accountLoading">一键注册官网账户</el-button>
          </el-empty>
          <template v-if="websiteAccount">
            ...
            <el-card shadow="never" style="margin-bottom: 16px">
              ...
              <el-row :gutter="12">
                <el-col :span="8"><el-button style="width: 100%" @click="handleResetPassword" :loading="accountLoading" type="warning" plain>重置密码</el-button></el-col>
                <el-col :span="8"><el-button style="width: 100%" @click="handleSendLoginCode" :loading="accountLoading" type="primary" plain>发送登录验证码</el-button></el-col>
                <el-col :span="8">
            ...
            <el-descriptions :column="2" border size="small" style="margin-bottom: 16px">
              ...
              <el-descriptions-item label="密码状态"><el-tag :type="websiteAccount.hasPassword ? 'success' : 'warning'" size="small">{{ websiteAccount.hasPassword ? '已设置' : '未设置' }}</el-tag></el-descriptions-item>
            ...
            <el-card shadow="never">
              ...
              <el-table :data="loginLogs" size="small" border max-height="300" v-loading="loginLogsLoading" empty-text="暂无登录记录">
                ...
                <el-table-column label="方式" width="100"><template #default="{ row }"><el-tag :type="row.loginMethod === 'sms_code' ? 'primary' : 'info'" size="small">{{ row.loginMethod === 'sms_code' ? '验证码登录' : '密码登录' }}</el-tag></template></el-table-column>
                <el-table-column label="结果" width="80"><template #default="{ row }"><el-tag :type="row.loginResult === 'success' ? 'success' : 'danger'" size="small">{{ row.loginResult === 'success' ? '成功' : '失败' }}</el-tag></template></el-table-column>
                <el-table-column label="失败原因" show-overflow-tooltip><template #default="{ row }">{{ row.failReason || '—' }}</template></el-table-column>
                <el-table-column label="IP" width="140"><template #default="{ row }">{{ row.ipAddress || '—' }}</template></el-table-column>
              </el-table>
            </el-card>
          </template>
        </el-tab-pane>
        ...
        <el-tab-pane label="会员等级" name="member">
          <el-card shadow="never" style="margin-bottom: 16px; background: #fafafa">
            ...
            <el-progress :percentage="upgradeProgress" :stroke-width="8" :status="upgradeProgress >= 100 ? 'success' : ''" />
            <div style="margin-top: 8px; color: #909399; font-size: 12px">成为会员: {{ detail.memberSince || '—' }} &nbsp;|&nbsp; 最后活跃: {{ detail.lastActiveAt || '—' }} &nbsp;|&nbsp; 有效期至: {{ detail.memberValidUntil || '—' }}</div>
          </el-card>
          <h4 style="margin: 12px 0 8px">等级变更记录</h4>
          <el-table :data="memberLevelLogs" size="small" border>
            <el-table-column label="变更" width="140"><template #default="{ row }"><el-tag size="small" type="info">{{ row.oldLevel?.toUpperCase() }}</el-tag> → <el-tag size="small" type="success">{{ row.newLevel?.toUpperCase() }}</el-tag></template></el-table-column>
            <el-table-column label="触发类型" width="100"><template #default="{ row }"><el-tag :type="triggerTypeTag(row.triggerType)" size="small">{{ triggerTypeLabel(row.triggerType) }}</el-tag></template></el-table-column>
          ...
          <el-table :data="pointsTransactions" size="small" border>
            <el-table-column label="积分" width="80" align="right"><template #default="{ row }"><span :style="{ color: row.points > 0 ? '#67C23A' : '#F56C6C', fontWeight: 'bold' }">{{ row.points > 0 ? '+' : '' }}{{ row.points }}</span></template></el-table-column>
            <el-table-column label="变更后余额" width="100" align="right"><template #default="{ row }">{{ row.balanceAfter }}</template></el-table-column>
            <el-table-column label="类型" width="120"><template #default="{ row }"><el-tag :type="pointsTypeTag(row.type)" size="small">{{ pointsTypeLabel(row.type) }}</el-tag></template></el-table-column>
      ...
      <el-tabs v-else v-model="activeTab" @tab-click="onTabClick">
        ...
        <el-tab-pane label="联系人" name="contacts">
          <el-table :data="contacts" size="small" border style="margin-bottom: 12px">
            ...
            <el-table-column label="主要" width="60"><template #default="{ row }">{{ row.isPrimary ? '主地址' : '' }}</template></el-table-column>
          </el-table>
          <el-button size="small" @click="openContactDialog()">+ 添加联系人</el-button>
        </el-tab-pane>
        <el-tab-pane label="地址" name="address">
          <el-table :data="addresses" size="small" border style="margin-bottom: 12px">
            <el-table-column prop="addressType" label="类型" width="80"><template #default="{ row }">{{ addrTypeLabel(row.addressType) }}</template></el-table-column>
            <el-table-column prop="province" label="省份" width="70" />
            <el-table-column prop="city" label="城市" width="80" />
            <el-table-column prop="detailAddress" label="详细地址" show-overflow-tooltip />
            <el-table-column prop="receiverName" label="收件人" width="80" />
            <el-table-column prop="receiverPhone" label="电话" width="110" />
            <el-table-column label="默认" width="60"><template #default="{ row }">{{ row.isDefault ? '默认' : '' }}</template></el-table-column>
            <el-table-column label="操作" width="120" fixed="right">
              <template #default="{ row }">
                <el-button link type="primary" size="small" @click="openEditAddress(row)">编辑</el-button>
                <el-popconfirm title="确认删除此地址？" @confirm="handleDeleteAddress(row.addressId)">
        ...
        <el-tab-pane label="阶梯定价" name="pricing">
          <el-table :data="tierPricings" size="small" border style="margin-bottom: 12px">
            ...
            <el-table-column prop="discountRate" label="折扣率" width="80"><template #default="{ row }">{{ (row.discountRate * 100).toFixed(0) }}%</template></el-table-column>
  ...
  <!-- 地址弹窗 -->
  <el-dialog v-model="addressDialogVisible" :title="isEditAddress ? '编辑地址' : '新增地址'" width="500px" destroy-on-close @closed="emit('refresh')">
    <el-form label-width="80px">
      <el-form-item label="类型"><el-select v-model="addressForm.addressType"><el-option label="收货" value="shipping" /><el-option label="结算" value="billing" /><el-option label="办公" value="office" /></el-select></el-form-item>
  ...
  <el-dialog v-model="tierDialogVisible" title="新增阶梯定价" width="500px" destroy-on-close @closed="emit('refresh')">
    <el-form label-width="80px">
      ...
      <el-form-item label="折扣率"><el-input-number v-model="tierForm.discountRate" :min="0" :max="1" :step="0.01" /></el-form-item>
    </el-form>
    <template #footer>
  ...
  <!-- 处方弹窗 -->
  <el-dialog v-model="prescriptionDialogVisible" title="新增处方" width="500px" destroy-on-close @closed="emit('refresh')">
    <el-form label-width="80px">
