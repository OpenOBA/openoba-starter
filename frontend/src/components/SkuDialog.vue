...
<template>
  ...
  >
    <el-tabs v-model="activeTab">
      ...
      <el-tab-pane label="基本信息" name="basic">
        <el-form ref="basicFormRef" :model="form" :rules="basicFormRules" label-width="110px" class="sku-tab-scroll">
          ...
          <div v-if="displayName" class="name-preview" style="margin-bottom: 16px">
            <span class="name-text">📋 展示名：<strong>{{ displayName }}</strong></span>
          ...
          <el-row :gutter="16">
            <el-col :span="12">
              <el-form-item label="SKU 编码"><el-input v-model="form.skuCode" disabled placeholder="自动生成" /></el-form-item>
            </el-col>
            <el-col :span="12">
          ...
          <el-form-item label="名称"><el-input v-model="form.skuName" placeholder="可选，系统自动生成展示名" /></el-form-item>
          ...
          <div class="effect-section">
            <div class="section-title">🎨 效果词（V3.0 命名规范 · 一维效果）<el-tag size="small" type="success" style="margin-left:8px">Schema</el-tag></div>
            <el-row :gutter="16">
              <el-col :span="12">
                ...
                <el-button link size="small" type="primary" @click="openEffectDict('skin_tone')" style="margin-top: 4px">📖 查看全部</el-button>
              </el-col>
              <el-col :span="12">
                ...
                <el-button link size="small" type="primary" @click="openEffectDict('face_shape')" style="margin-top: 4px">📖 查看全部</el-button>
              </el-col>
            </el-row>
          </div>
          ...
          <!-- 价格行 -->
          <el-row :gutter="16">
            <el-col :span="8" class="w-short"><el-form-item label="统一零售价" prop="retailPrice"><el-input-number v-model="form.retailPrice" :precision="2" :min="0" /></el-form-item></el-col>
            <el-col :span="8" class="w-short"><el-form-item label="成本价"><el-input-number v-model="form.costPrice" :precision="2" :min="0" /></el-form-item></el-col>
            <el-col :span="8" class="w-short"><el-form-item label="最低售价"><el-input-number v-model="form.minPrice" :precision="2" :min="0" /></el-form-item></el-col>
          </el-row>

          <el-row :gutter="16">
            <el-col :span="12" class="w-short"><el-form-item label="库存"><el-input-number v-model="form.stockQuantity" :min="0" /></el-form-item></el-col>
            <el-col :span="12" class="w-short"><el-form-item label="EAN-13"><el-input v-model="form.ean13" maxlength="13" placeholder="自动生成" /></el-form-item></el-col>
          </el-row>

          <el-form-item label="内部条码" class="w-barcode">
            <el-input v-model="form.skuBarcode" readonly />
            <el-button size="small" @click="genBarcode" style="margin-left: 8px">生成</el-button>
          </el-form-item>
        </el-form>
      </el-tab-pane>
      ...
      <el-tab-pane label="技术参数" name="tech">
        <el-form :model="form" label-width="110px" style="max-height: 500px; overflow-y: auto; padding: 12px 0">
          ...
          <el-row :gutter="16">
            <el-col :span="8"><el-form-item label="镜片宽度"><el-input :model-value="inheritedLensWidth" disabled /><div style="font-size:11px;color:#909399;line-height:1.4">继承自结构标准</div></el-form-item></el-col>
            <el-col :span="8"><el-form-item label="鼻梁宽度"><el-input :model-value="inheritedBridgeWidth" disabled /><div style="font-size:11px;color:#909399;line-height:1.4">继承自结构标准</div></el-form-item></el-col>
            <el-col :span="8"><el-form-item label="镜腿长度"><el-input-number v-model="form.templeLength" :min="120" :max="160" style="width:100%" placeholder="填写" /></el-form-item></el-col>
          </el-row>
          ...
          <el-row :gutter="16">
            <el-col :span="6"><el-form-item label="防蓝光"><el-switch v-model="form.hasBlueLightFilter" /></el-form-item></el-col>
            <el-col :span="6"><el-form-item label="变色"><el-switch v-model="form.hasPhotochromic" /></el-form-item></el-col>
            <el-col :span="6"><el-form-item label="偏光"><el-switch v-model="form.hasPolarized" /></el-form-item></el-col>
            <el-col :span="6">
              <el-form-item label="UV防护">
                <el-select v-model="form.uvProtection">
                  <el-option v-for="opt in uvProtectionOptions" :key="opt.value" :label="opt.label" :value="opt.value" />
                </el-select>
              </el-form-item>
            </el-col>
          </el-row>
    ...
    <!-- 效果词库弹窗 -->
    <el-dialog v-model="effectDictVisible" :title="dictType === 'skin_tone' ? '🎨 肤色效果词库' : '🎨 脸型效果词库'" width="680px" append-to-body>
...
// Dialog打开时自动加载色彩列表
watch(() => props.visible, (v) => { if (v && !colorList.value.length) loadColors(); });
...
const uvProtectionOptions = computed(() => {
  return props.schemaConfig?.uvProtectionOptions || [
...
const effectiveTierList = computed(() => {
  ...
  if (configTiers && Object.keys(configTiers).length) {
    return Object.entries(configTiers).map(([code, info]) => ({ tier_code: code, tier_name: info.name, icon_color: info.color }))
  }
  return Object.entries(TIER_DEFAULT_MAP).map(([code, info]) => ({ tier_code: code, tier_name: info.name, icon_color: info.color }))
})
...
const displayName = computed(() => {
  ...
  if (!spu) return ''
  const struct = props.structureStandards.find(l => l.internalCode === (form.structureStandardCode || spu.structureStandardCode))
  if (!struct) return ''
...
const dictItems = computed(() => {
  ...
  const defaults: Record<string, string[]> = {
    skin_tone: ['冷白皮提气', '黄皮肤增白', '中性百搭', '暖皮显气质', '黄皮肤提亮', '暖皮提亮', '冷白皮显白', '暖皮显白', '黄皮肤显白', '冷白皮显嫩'],
...
async function onColorChange(colorCode: string) {
  ...
  if (colorCode && !isEdit.value) {
    ...
    } catch (e) { console.warn('[SkuDialog] getEffectRecommend failed for', colorCode, e) }
  }
}
...
function resetForm() {
  Object.assign(form, {
    skuId: '', skuCode: '', skuName: '', spuId: '', spu: null, colorCode: '',
    productTier: '', structureStandardCode: '', skinToneEffect: '', faceShapeEffect: '',
    retailPrice: null, costPrice: null, minPrice: null, stockQuantity: null,
    ean13: '', skuBarcode: '',
    frameMaterial: '', templeMaterial: '', frameType: '', surfaceTreatment: '',
    templeLength: null, nosePadType: '', hingeType: '', weightG: null,
    suitableFaceShapes: [], hasBlueLightFilter: false, hasPhotochromic: false,
    hasPolarized: false, uvProtection: 'None',
  })
...
async function handleSave() {
  ...
  try {
    ...
    delete saveData.lensWidth; delete saveData.bridgeWidth
...
.effect-tag:hover { border-color: #409eff; color: #409eff; }
.effect-tag.active { background: #409eff; color: #fff; border-color: #409eff; }
.effect-dict-panel h4 { margin-bottom: 16px; color: #303133; }
.effect-dict-grid { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 12px; margin-bottom: 16px; }
.effect-dict-item {
  padding: 10px; border: 1px solid #dcdfe6; border-radius: 6px;
  cursor: pointer; transition: all 0.2s;
}
.effect-dict-item:hover { border-color: #409eff; box-shadow: 0 2px 8px rgba(64,158,255,0.15); }
.effect-dict-item .name { font-weight: 600; font-size: 14px; }
.effect-dict-item .target { font-size: 11px; color: #909399; margin-top: 4px; }
</style>
