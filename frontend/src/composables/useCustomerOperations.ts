...
export function useCustomerOperations(
  ...
  function openContactDialog() {
    Object.assign(contactForm, { contactName: '', role: '', phone: '', email: '', wechat: '', isPrimary: false })
  ...
  function openEditAddress(row: Record<string, unknown>) {
    ...
    editingAddressId.value = row.addressId as string
    addressDialogVisible.value = true
  }
  async function handleSaveAddress() {
    if (isEditAddress.value) {
      ...
      await addAddress({ ...addressForm, customerId: detail.value!.customerId as string })
      ElMessage.success('地址已添加')
    }
    addresses.value = await getAddresses(detail.value!.customerId as string)
  ...
  function openTierPricingDialog() {
    Object.assign(tierForm, { tierName: '', minQty: 1, maxQty: 9999, discountRate: 1.0, notes: '' })
    tierDialogVisible.value = true
  }
  async function handleAddTierPricing() {
    await addTierPricing({ ...tierForm, customerId: detail.value!.customerId as string })
  ...
  async function handleAddPrescription() {
    await addPrescription({ ...prescriptionForm, customerId: detail.value!.customerId as string })
    ElMessage.success('处方已添加')
    prescriptions.value = await getPrescriptions(detail.value!.customerId as string)
  ...
  function openLensDialog() {
    Object.assign(lensForm, {
      customerId: detail.value!.customerId as string,
      lensStandardCode: '',
      prescriptionId: '',
      purchaseDate: '',
      orderId: '',
  ...
  async function handleAddLens() {
    ...
    customerLenses.value = await getCustomerLenses(detail.value!.customerId as string)
    try {
      lensSummary.value = await getCustomerLensSummary(detail.value!.customerId as string)
    } catch {
      /* ignore */
    }
  ...
  async function loadWebsiteAccount() {
    ...
    try {
      websiteAccount.value = await getWebsiteAccount(detail.value.customerId as string)
      if (websiteAccount.value?.accountStatus !== 'none') loadLoginLogs()
    } catch {
      websiteAccount.value = null
    }
  }
  async function loadLoginLogs() {
    ...
    try {
      const res = (await getLoginLogs(detail.value.customerId as string, 20)) as unknown as {
  ...
  async function handleRegisterAccount() {
    ...
    try {
      await registerWebsiteAccount(detail.value.customerId as string)
      ElMessage.success({ message: '官网账户已创建！', duration: 6000, showClose: true })
  ...
  async function handleResetPassword() {
    ...
    accountLoading.value = true
    try {
      await resetPassword(detail.value.customerId as string)
      ElMessage.success({ message: '密码已重置', duration: 6000, showClose: true })
    } catch (e: unknown) {
      const err = e instanceof Error ? e.message : String(e)
      ElMessage.error(err || '重置失败')
    } finally {
      accountLoading.value = false
    }
  }
  async function handleSendLoginCode() {
    ...
    accountLoading.value = true
    try {
      const res = await sendLoginCode(detail.value.customerId as string)
  ...
  async function handleStatusChange(status: string) {
    if (!detail.value) return
    try {
      const res = await toggleAccountStatus(detail.value.customerId as string, status)
