    ...
    <el-card class="mt-16">
      ...
      <el-table v-loading="loading" :data="tableData" stripe border>
        ...
        <el-table-column label="操作" width="240" fixed="right">
          <template #default="{ row }">
            <el-button
              v-if="row.status === AFTER_SALES_STATUS.pending"
              size="small"
              type="success"
            ...
            >
            <el-button
              v-if="row.status === 'approved' || row.status === 'returning'"
              size="small"
              @click="doProcess(row, 'receive')"
              >确认收货</el-button
            >
            <el-button
              v-if="row.status === 'received' || row.status === 'approved'"
              size="small"
              type="primary"
    ...
    <el-dialog v-model="detailVisible" title="售后详情" width="800px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="状态">
          <el-tag :type="statusTag(detailData.status as string)" size="small">{{
            statusLabel(detailData.status as string)
          }}</el-tag>
      ...
      <el-timeline>
        <el-timeline-item v-for="log in detailLogs" :key="log.id" :timestamp="log.createdAt" placement="top">
...
function showReviewDialog(row: Record<string, unknown>, action: string) {
  ...
  reviewForm.actualRefundAmount = parseFloat(row.refundAmount as string) || 0
...
function showCreateDialog() {
  createForm.orderId = ''
  createForm.reasonDetail = ''
  createForm.refundAmount = 0
  createVisible.value = true
}
async function doCreate() {
  try {
    await createAfterSales(createForm)
...
function statusLabel(status: string) {
  return (
    afterSaleStatusDict.labels.value[status] ||
    {
      pending: '待审核',
      approved: '已批准',
      rejected: '已拒绝',
