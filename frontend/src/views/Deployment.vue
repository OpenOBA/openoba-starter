<template>
 ...
 <el-button size="small" :type="deployMode.mode === 'operator' ? 'primary' : 'default'" @click="switchMode('operator')" :loading="switchingMode === 'operator'"> 运营模式</el-button>
 <el-button size="small" :type="deployMode.mode === 'developer' ? 'warning' : 'default'" @click="switchMode('developer')" :loading="switchingMode === 'developer'"> 开发模式</el-button>
 <el-button size="small" :type="deployMode.mode === 'maintainer' ? 'danger' : 'default'" @click="switchMode('maintainer')" :loading="switchingMode === 'maintainer'"> 维护模式</el-button>
 ...
 <el-space>
 <el-button type="primary" @click="handleSync" :loading="syncing" :disabled="status?.synced">
 同步 Staging → Production
 </el-button>
 <el-button @click="loadStatus"> 刷新状态</el-button>
 <el-button type="warning" @click="handleStartStaging" :disabled="status?.staging?.running">
 启动 Staging
 </el-button>
 ...
 <el-table :data="deltas" stripe border size="small" v-loading="loading">
 ...
 <template #default="{ row }">
 <el-button v-if="row.status === 'pending_staging'" size="small" type="primary" @click="deployStaging(row.id)">
 部署到 Staging
 </el-button>
 ...
 <el-button v-if="row.status === 'promoted'" size="small" disabled>
 已上线
 </el-button>
 <el-button v-if="row.status === 'rolled_back'" size="small" disabled>
 已回滚
 </el-button>
 </template>
 </el-table-column>
 ...
 <!-- 回滚面板 -->
 <el-card shadow="hover" style="margin-top:20px" v-if="rollbackHistory.length > 0">
...
const deployMode = reactive({
 mode: 'operator', label: '运营模式', desc: '', restrictions: [] as string[],
})
const switchingMode = ref('')
const modeIcon = computed(() => deployMode.mode === 'operator' ? '' : deployMode.mode === 'developer' ? '' : '')
const modeTagType = computed(() => deployMode.mode === 'operator' ? 'primary' : deployMode.mode === 'developer' ? 'warning' : 'danger')
...
function statusLabel(s: string) {
 ...
 pending_staging: '待部署', on_staging: '测试中', verified: '已验收',
 promoted: '已上线', rolled_back: '已回滚',
 }
 return map[s] || s
}
...
function statusTagType(s: string) {
 const map: Record<string, string> = {
 pending_staging: 'info', on_staging: 'warning', verified: 'success',
 promoted: '', rolled_back: 'danger',
 }
 return map[s] || 'info'
}
...
onMounted(() => { loadStatus(); loadDeployMode() })
</script>

<style scoped>
.deployment-page { padding: 16px; }
.deployment-page h2 { margin: 0 0 20px; color: #303133; }
.stat-card { text-align: center; }
.stat-value { font-size: 24px; font-weight: 700; color: #303133; margin-bottom: 4px; }
.stat-label { font-size: 13px; color: #909399; }
.stat-card-warn { border: 1px solid #e6a23c; }
.mode-card-danger { border-left: 3px solid #f56c6c; background: #fef0f0; }</style>
