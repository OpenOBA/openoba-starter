...
const filters = ref<any>({ page: 1, pageSize: 20 });
...
async function batchApprove() {
  ...
  let ok = 0;
  for (const d of selectedDrafts.value) { try { await doReview(d, 'approve'); ok++ } catch {} }
...
async function batchReject() {
  ...
  try {
    ...
    for (const d of selectedDrafts.value) {
      try { await doReview(d, 'reject', value); ok++ } catch {}
    }
    ElMessage.success(`完成: ${ok}/${selectedDrafts.value.length}`)
    selectedDrafts.value = []; fetchDrafts();
  } catch { /* 用户取消 */ }
...
const detailDraft = ref<any>(null);
const detailSkus = ref<any[]>([]);
const editForm = ref<any>({});
const saving = ref(false);

const draftForm = ref<any>({ gender: 'female', shapeCode: '', seriesCode: '', structureStandardCode: 'SL-001', spuName: '', skusJson: '' });
const batchForm = ref<any>({ name: '', type: 'ai' });
...
function statusLabel(s: string) {
  const m: any = { draft:'草稿', reviewed:'已审', approved:'通过', published:'已发布', rejected:'已驳回' };
  return m[s] || s;
}
function statusTag(s: string) {
  const m: any = { draft:'info', reviewed:'warning', approved:'success', published:'success', rejected:'danger' };
  return m[s] || 'info';
}
function pkgStatusTag(s: string) {
  const m: any = { processing:'warning', completed:'success', failed:'danger' };
  return m[s] || 'info';
}
...
async function submitDraft() {
  try {
    let skus = [];
    if (draftForm.value.skusJson) skus = JSON.parse(draftForm.value.skusJson);
    await createDraftSpu({ ...draftForm.value, skus });
...
async function submitBatch() {
  try {
    await createBatch(batchForm.value.name, batchForm.value.type);
...
async function finishBatch(id: string) {
  try { await completeBatch(id); ElMessage.success('已完成'); fetchBatches(); } catch {}
}

async function doReview(row: any, action: string, reason?: string) {
  try {
    const payload = { action, rejectedReason: reason } as const;
    await reviewDraftApi(row.draftId, { ...payload, skuIds: undefined });
...
async function openDetail(row: Record<string, unknown>) {
  try {
    const r = await getDraftDetail(String(row.draftId ?? ''));
...
async function saveDetail() {
  ...
  try {
    await updateDraft(detailDraft.value.draftId, {
      ...editForm.value,
      skus,
    });
