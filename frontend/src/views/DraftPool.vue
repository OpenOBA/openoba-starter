<template>
  <div class="draft-pool-container">
    ...
    <el-tabs v-model="activeTab" @tab-click="onTabChange">
      <el-tab-pane label="草稿列表" name="drafts">
        ...
        <el-form :inline="true" size="small" class="filter-bar">
          <el-form-item label="状态"
            ><el-select
              ...
              @change="fetchDrafts"
            >
              <el-option v-for="s in statusOptions" :key="s.value" :label="s.label" :value="s.value" /> </el-select
          ></el-form-item>
          <el-form-item label="性别"
            ><el-select
              ...
              @change="fetchDrafts"
            >
              <el-option label="女款" value="female" /><el-option label="男款" value="male" /> </el-select
          ></el-form-item>
          <el-form-item label="造型"
            ><el-select
              v-model="filters.shapeCode"
              clearable
              placeholder="全部"
              style="width: 120px"
              @change="fetchDrafts"
            >
              <el-option v-for="s in dictLensShape" :key="s" :label="s" :value="s" /> </el-select
          ></el-form-item>
          <el-form-item label="系列"
            ><el-select
              v-model="filters.seriesCode"
              clearable
              placeholder="全部"
              style="width: 120px"
              @change="fetchDrafts"
            >
              <el-option v-for="s in dictLensSeries" :key="s" :label="s" :value="s" /> </el-select
          ></el-form-item>
          <el-form-item label="来源"
            ><el-select
              ...
              @change="fetchDrafts"
            >
              <el-option label="AI生成" value="ai" /><el-option label="手动" value="manual" /> </el-select
          ></el-form-item>
        ...
        <div class="batch-actions" style="margin-bottom: 8px; display: flex; align-items: center; gap: 12px">
          <span style="color: #909399; font-size: 13px">{{
            selectedDrafts.length > 0 ? `已选 ${selectedDrafts.length} 项` : '勾选后可批量操作'
          }}</span>
          ...
          <el-button size="small" type="danger" plain :disabled="selectedDrafts.length === 0" @click="batchDelete"
            >🗑 删除</el-button
          >
        </div>
        <el-table
        ...
        >
          ...
          <el-table-column label="状态" width="100">
            <template #default="{ row }"
              ><el-tag :type="statusTag(row.status)" size="small">{{ statusLabel(row.status) }}</el-tag></template
            >
          </el-table-column>
          <el-table-column prop="aestheticLevel" label="美学" width="80">
            <template #default="{ row }">
              <el-tag v-if="row.aestheticLevel === 'pass'" type="success" size="small"
                >✅{{ row.aestheticScore || '' }}</el-tag
              >
      ...
      <el-tab-pane label="批次管理" name="batches">
        ...
        <el-table :data="batchList" stripe border size="small" class="mt-2">
          ...
          <el-table-column label="操作" width="100">
            <template #default="{ row }">
              <el-button v-if="row.status === 'generating'" size="small" @click="finishBatch(row.batchId)"
                >完成</el-button
              >
            </template>
          </el-table-column>
      ...
      <el-tab-pane label="发布记录" name="packages">
        <el-table :data="pkgList" stripe border size="small">
          ...
          <el-table-column prop="status" label="状态" width="100">
            <template #default="{ row }"
              ><el-tag :type="pkgStatusTag(row.status)" size="small">{{ row.status }}</el-tag></template
            >
          </el-table-column>
        ...
        <el-row :gutter="16">
          <el-col :span="8"
            ><el-form-item label="性别" required
              ><el-select v-model="draftForm.gender" style="width: 100%">
          ...
          ></el-col>
          <el-col :span="8"
            ><el-form-item label="造型" required
              ><el-select v-model="draftForm.shapeCode" style="width: 100%">
                <el-option v-for="s in dictLensShape" :key="s" :label="s" :value="s" /> </el-select></el-form-item
          ></el-col>
          <el-col :span="8"
            ><el-form-item label="系列" required
              ><el-select v-model="draftForm.seriesCode" style="width: 100%">
                <el-option v-for="s in dictLensSeries" :key="s" :label="s" :value="s" /> </el-select></el-form-item
          ></el-col>
        </el-row>
        <el-form-item label="结构标准" required
          ><el-input v-model="draftForm.structureStandardCode" placeholder="SL-001"
        /></el-form-item>
        <el-form-item label="SKU (JSON数组)"
          ><el-input
            v-model="draftForm.skusJson"
            type="textarea"
            :rows="4"
            placeholder='[{"colorCode":"macaron_pink","skinToneEffect":"yellow_skin_whiten","faceShapeEffect":"round_face_slim"}]'
        /></el-form-item>
      </el-form>
      <template #footer>
      ...
      <el-form :model="batchForm" label-width="100px">
        ...
        <el-form-item label="生成方式"
          ><el-select v-model="batchForm.type">
            <el-option label="AI 生成" value="ai" /><el-option label="手动导入" value="manual" /> </el-select
        ></el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog v-model="showWaitlist" title="待审核队列" width="700px">
      ...
      <el-table
        :data="draftList.filter((d) => d.status === 'draft' || d.status === 'reviewed')"
        stripe
        border
        size="small"
        class="mt-2"
      ...
      <template v-if="detailDraft">
        <el-form :model="editForm" label-width="100px" size="small">
          ...
          <el-form-item label="描述">
            <el-input v-model="editForm.spuDescription" type="textarea" :rows="2" />
          </el-form-item>
          <el-row :gutter="12">
            <el-col :span="8">
              <el-form-item label="性别">
                <el-select v-model="editForm.gender" style="width: 100%">
                  <el-option label="女款" value="female" /><el-option label="男款" value="male" /><el-option
                    label="中性"
                    value="unisex"
                  />
                </el-select>
...
const canApprove = computed(
  () =>
    selectedDrafts.value.length > 0 && selectedDrafts.value.every((d: Record<string, unknown>) => d.status === 'draft'),
)
...
const canReview = computed(
  () =>
    ...
    selectedDrafts.value.every((d: Record<string, unknown>) => d.status === 'reviewed'),
)

async function batchApprove() {
  if (!canApprove.value) {
    ElMessage.warning('仅可对「草稿」状态的项执行通过')
    return
  }
  try {
    await ElMessageBox.confirm(`确定通过选中的 ${selectedDrafts.value.length} 个草稿？`, '批量通过', { type: 'info' })
  } catch {
    return
  }
  let ok = 0
...
async function batchPublish() {
  if (!canReview.value) {
    ElMessage.warning('仅可对「已审」状态的项执行发布')
    return
  }
  try {
    await ElMessageBox.confirm(`确定发布选中的 ${selectedDrafts.value.length} 个草稿？`, '批量发布', { type: 'info' })
  } catch {
    return
  }
  const ids = selectedDrafts.value.map((d) => d.draftId as string)
  try {
    await publishDrafts({ draftIds: ids })
...
async function batchReject() {
  if (!canReject.value) {
    ElMessage.warning('仅可对「草稿」状态的项执行驳回')
    return
  }
  try {
...
const draftForm = ref<Record<string, unknown>>({
  gender: 'female',
  shapeCode: '',
  seriesCode: '',
  structureStandardCode: 'SL-001',
  spuName: '',
  skusJson: '',
})
...
const shapeCnMap: Record<string, string> = {
  RND: '圆形',
  SQR: '方形',
  OVL: '椭圆',
  CAT: '猫眼',
...
async function fetchWaitlist() {
  try {
    waitlist.value = await getWaitlistCount()
...
async function fetchBatches() {
  try {
    batchList.value = (await getBatches()) as unknown as Record<string, unknown>[]
  } catch {
    /* ignore */
  }
}
async function fetchPackages() {
  try {
    pkgList.value = (await getPackages()) as unknown as Record<string, unknown>[]
  } catch {
    /* ignore */
  }
}
...
async function openDetail(row: Record<string, unknown>) {
  try {
    const r = (await getDraftDetail(String(row.draftId ?? ''))) as unknown as {
