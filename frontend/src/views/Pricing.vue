...
const handleSaveTier = async () => { try { if (tierForm.tierId) { await updateTierPricing(tierForm.tierId, { tierName: tierForm.tierName, positioning: tierForm.positioning, sortOrder: tierForm.sortOrder }); ElMessage.success('更新成功') } else { tierForm.tierId = 'tier-' + Date.now() + '-' + Math.random().toString(36).slice(2, 8); await createTierPricing({ ...tierForm, isActive: true }); ElMessage.success('创建成功') } tierDialogVisible.value = false; selectedTiers.value = []; await loadTiers() } catch (e: unknown) { ElMessage.error('保存失败: ' + ((e as any)?.response?.data?.message || (e as Error).message)) } }
