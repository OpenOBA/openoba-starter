    ...
    <el-table v-loading="loading" :data="reviewList" stripe>
      <el-table-column label="评分" width="100">
        <template #default="{ row }">
          <div class="score-cell">
            <span
              class="score-num"
              :style="{ color: row.overallScore >= 4 ? '#67c23a' : row.overallScore >= 3 ? '#e6a23c' : '#f56c6c' }"
            >
              {{ row.overallScore }}
            </span>
      ...
      <el-table-column label="操作" width="240" fixed="right">
        <template #default="{ row }">
          ...
          <el-button v-if="row.status === REVIEW_STATUS.pending" link type="success" @click="handleApprove(row)"
            >通过</el-button
          >
          <el-button v-if="row.status === REVIEW_STATUS.pending" link type="warning" @click="handleReject(row)"
            >拒绝</el-button
          >
      ...
      <div v-if="currentReview" class="review-detail">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="总评分">
            <span :style="{ color: (currentReview.overallScore as number) >= 4 ? '#67c23a' : '#f56c6c' }"
              >{{ currentReview.overallScore }} / 5</span
            >
          ...
          <el-descriptions-item label="状态"
            ><el-tag :type="statusType(currentReview.status as string)" size="small">{{
              statusLabel(currentReview.status as string)
          ...
          <el-descriptions-item label="标签" :span="2">
            <el-tag v-for="tag in currentReview.tags || []" :key="tag" size="small" style="margin: 2px">{{
              tag
            }}</el-tag>
          </el-descriptions-item>
          <el-descriptions-item v-if="currentReview.replyContent" label="商家回复" :span="2">
            <div class="reply-box">
              ...
              <div v-if="currentReview.replyAt" class="reply-time">{{ currentReview.replyAt }}</div>
            </div>
          </el-descriptions-item>
        </el-descriptions>
      </div>
...
const loadReviews = async () => {
  loading.value = true
  try {
    const res = await getReviews({ ...filters, page: page.value, pageSize } as Record<
      string,
...
const updateStats = () => {
  ...
  stats.avgScore =
    approved.length > 0
      ? (
          approved.reduce((s: number, r: Record<string, unknown>) => s + (r.overallScore as number), 0) /
          approved.length
        ).toFixed(1)
      : '-'
  const today = new Date().toISOString().slice(0, 10)
  stats.todayCount = reviewList.value.filter((r: Record<string, unknown>) =>
...
const statusLabel = (s: string) =>
  reviewStatusDict.labels.value[s] || { pending: '待审核', approved: '已通过', rejected: '已拒绝' }[s] || s
...
const handleApprove = async (row: Record<string, unknown>) => {
  try {
    await reviewAction(String(row.reviewId ?? ''), { action: 'approve' })
...
const handleReject = async (row: Record<string, unknown>) => {
  try {
    await reviewAction(String(row.reviewId ?? ''), { action: 'reject' })
...
const openReplyDialog = (row: Record<string, unknown>) => {
...
const handleReply = async () => {
  ...
  try {
    await replyReview(replyForm.reviewId, { content: replyForm.content, replyBy: 'admin' })
