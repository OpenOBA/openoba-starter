    ...
    <el-tabs v-model="activeTab" type="border-card">
      ...
      <el-tab-pane label="用户管理" name="users">
        <div class="toolbar">
          ...
          <el-button type="primary" style="margin-left: auto" @click="openUserDialog()">新增用户</el-button>
        </div>
        <el-table v-loading="userLoading" :data="userList" stripe style="margin-top: 12px">
      ...
      <el-tab-pane label="角色管理" name="roles">
        ...
        <el-table v-loading="roleLoading" :data="roleList" stripe style="margin-top: 12px">
      ...
      <el-tab-pane label="审计日志" name="audit">
        <div class="toolbar">
          ...
          <el-select
            v-model="auditFilter.sensitivity"
            placeholder="敏感级别"
            clearable
            style="width: 140px; margin-left: 8px"
          >
        ...
        <el-table v-loading="auditLoading" :data="auditList" stripe style="margin-top: 12px" max-height="520">
          <el-table-column prop="actionTime" label="时间" width="180" />
          <el-table-column label="操作者" width="140">
            <template #default="{ row }">
              <span>{{ row.actorName || row.actorId }}</span>
              <el-tag size="small" :type="row.actorType === 'agent' ? 'warning' : ''" style="margin-left: 4px">{{
                row.actorType
              }}</el-tag>
            </template>
          </el-table-column>
          ...
          <el-table-column label="敏感" width="70">
            <template #default="{ row }">
              <el-icon
                v-if="row.sensitivity !== 'none'"
                :color="row.sensitivity === 'confidential' ? '#f56c6c' : '#e6a23c'"
                ><WarningFilled
              /></el-icon>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="result" label="结果" width="80">
            <template #default="{ row }">
              <el-tag
                :type="row.result === 'success' ? 'success' : row.result === 'blocked' ? 'danger' : 'warning'"
                size="small"
                >{{ row.result }}</el-tag
              >
            </template>
