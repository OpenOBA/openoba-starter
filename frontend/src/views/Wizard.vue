  ...
  <div class="wizard-page">
    ...
    <div class="wizard-steps-bar">
      <div
        ...
        :key="idx"
        class="step-node"
        :class="{ done: step.status === 'done', active: step.status === 'active', failed: step.status === 'failed' }"
      >
        <div class="step-dot">
    ...
    <div class="wizard-body">
      ...
      <div v-if="currentStep === 0" class="step-panel">
        ...
        <el-form :model="db" label-width="100px" size="default">
          ...
          <el-form-item label="密码"
            ><el-input v-model="db.password" type="password" placeholder="输入 MySQL 密码" show-password
          /></el-form-item>
          ...
          <el-form-item>
            ...
            <span v-if="dbResult" :class="dbResult.success ? 'ok' : 'fail'"
              >{{ dbResult.success ? '✅' : '❌' }} {{ dbResult.message }}</span
            >
          </el-form-item>
        </el-form>
        <div class="step-actions">
          <el-button type="primary" size="large" :disabled="!dbConnected" @click="goStep(1)">下一步 →</el-button>
        </div>
      </div>
      ...
      <div v-if="currentStep === 1" class="step-panel">
        ...
        <div v-if="!step2Done" class="init-area">
          <el-button type="primary" size="large" :loading="creating" :disabled="creating" @click="createTables"
            >🏗️ 开始建表</el-button
          >
        ...
        <div class="step-actions">
          ...
          <el-button type="primary" size="large" :disabled="!step2Done" @click="goStep(2)">下一步 →</el-button>
        </div>
      </div>
      ...
      <div v-if="currentStep === 2" class="step-panel">
        ...
        <div v-if="!step3Done" class="init-area">
          <el-button type="primary" size="large" :loading="seeding" :disabled="seeding" @click="seedData"
            >🌱 导入种子数据</el-button
          >
        ...
        <div class="step-actions">
          ...
          <el-button type="primary" size="large" :disabled="!step3Done" @click="goStep(3)">下一步 →</el-button>
        </div>
      </div>
      ...
      <div v-if="currentStep === 3" class="step-panel">
        ...
        <div class="done-box">
          <p>
            <strong>登录地址：</strong><code>{{ loginUrl }}</code>
          </p>
          <p><strong>用户名：</strong>admin</p>
          <p><strong>密码：</strong>admin123</p>
          <p class="warn">⚠️ 首次登录后请立即修改密码！</p>
          <p class="hint">
            💡 DeepSeek API Key 新用户免费送 500 万 token：<a
...
async function createTables() {
  ...
  try {
    ...
    const interval = setInterval(() => {
      if (tableProgress.value < 90) {
        const arr = new Uint32Array(1)
        crypto.getRandomValues(arr)
        tableProgress.value += arr[0] % 15
        tableProgressText.value = `正在创建表 (${Math.floor(tableProgress.value)}%)...`
      }
    }, 300)
    const raw = await res.json()
  ...
  try {
    const interval = setInterval(() => {
      if (seedProgress.value < 90) {
        seedProgress.value += 20
        seedProgressText.value = '正在导入...'
      }
    }, 400)
