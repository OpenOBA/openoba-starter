  ...
  <div class="erdl-playground">
    ...
    <el-card shadow="hover" class="nl-generate-card">
      ...
      <div class="nl-input-row">
        ...
        <el-button type="primary" :loading="generating" :disabled="!nlPrompt.trim()" @click="generateERDL">
          生成 ERDL
        </el-button>
      </div>
      <div v-if="nlHint" class="nl-hint">
        <el-text type="info" size="small"> 试试说：{{ randomHint }} </el-text>
      </div>
    </el-card>
    ...
    <el-row :gutter="16" style="margin-top: 16px">
      ...
      <el-col :span="14">
        <el-card shadow="hover">
          ...
          <div class="toolbar">
            <el-button type="primary" :loading="validating" @click="validateYaml"> 校验语法 </el-button>
            <el-button type="success" :loading="loading" @click="loadYaml"> 加载到系统 </el-button>
            <el-button v-if="yamlCode" type="warning" text :disabled="!generatedValid" @click="loadGeneratedYaml">
              加载生成结果
            </el-button>
          </div>
        </el-card>
      ...
      <el-col :span="10">
        <el-card shadow="hover" class="result-panel">
          ...
          <!-- 生成状态 -->
          <el-alert v-if="generatedYaml && !errors.length" type="info" :closable="false" show-icon class="mb-3">
            <template #title> AI 已生成 ERDL YAML（{{ generatedValid ? '语法正确' : '需手动调整' }}）</template>
          </el-alert>

          <!-- 错误提示 -->
          <el-alert v-if="errors.length" type="error" :closable="false" show-icon class="mb-3">
            <template #title>语法错误</template>
            <div v-for="(err, i) in errors" :key="i" class="error-item">{{ err }}</div>
          </el-alert>

          <!-- 校验成功 -->
          <el-alert v-else-if="validated && !loading" type="success" :closable="false" show-icon class="mb-3">
...
.nl-generate-card :deep(.el-card__header) {
  padding: 12px 16px;
