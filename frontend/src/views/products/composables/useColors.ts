...
export function useColors() {
  ...
  const loadDicts = async () => {
    try {
      materials.value = await getDictLensMaterials()
    } catch {
      /* ignore */
    }
    try {
      const r = await getDictSkuColors()
      spuColors.value = r || []
    } catch {
      /* ignore */
    }
  ...
  const colorForm = reactive<ColorForm>({
    colorId: '',
    colorCode: '',
    colorName: '',
    colorNameEn: '',
  ...
  const loadColors = async () => {
    ...
    try {
      ...
      if (Array.isArray(res.items)) {
        colorList.value = res.items.filter((c) => c.colorId)
      }
    } catch (e: unknown) {
  ...
  const COLOR_DEFAULTS: ColorForm = {
    colorId: '',
    colorCode: '',
    colorName: '',
    colorNameEn: '',
  ...
  const onMappingPageChange = (p: number) => {
    mappingPage.value = p
    loadMappings()
  }

  const MAPPING_DEFAULTS: MappingForm = {
    materialCode: '',
    colorCode: '',
    feasibility: 'feasible',
    craftProcess: '',
  ...
  const paletteForm = reactive<PaletteForm>({
    season: 'SS26',
    paletteName: '',
    theme: '',
    targetAudience: '',
  ...
  const onPalettePageChange = (p: number) => {
    palettePage.value = p
    loadPalettes()
  }

  const PALETTE_DEFAULTS: PaletteForm = {
    season: 'SS26',
    paletteName: '',
    theme: '',
    targetAudience: '',
  ...
  const paletteItemForm = reactive<PaletteItemForm>({
    paletteId: '',
    colorCode: '',
    roleInPalette: 'primary',
    sortOrder: 0,
    notes: '',
  })
  ...
  const openAddPaletteItem = () => {
    ...
    Object.assign(paletteItemForm, {
      paletteId: currentPalette.value.paletteId as string,
      colorCode: '',
      roleInPalette: 'primary',
      sortOrder: currentPaletteItems.value.length,
      notes: '',
    })
  ...
  const projectStatusType = (s: string) =>
    ({ draft: 'info', designing: '', reviewing: 'warning', approved: 'success', production: '', archived: 'info' })[
      s
    ] || ''
  ...
  const onProjectPageChange = (p: number) => {
    projectPage.value = p
    loadProjects()
  }

  const openProjectDialog = (row?: Record<string, unknown>) => {
    const PROJECT_DEFAULTS: ProjectForm = {
      projectCode: '',
      projectName: '',
      description: '',
      targetSeason: '',
  ...
  const projectColorForm = reactive<ProjectColorForm>({
    projectId: '',
    colorCode: '',
    materialCode: '',
    isPrimary: false,
  ...
  const openProjectColorDialog = async (row: Record<string, unknown>) => {
    ...
    try {
      currentProjectColors.value = await getProjectColors(row.projectId as string)
    } catch {
      currentProjectColors.value = []
    }
  ...
  const openAddProjectColor = () => {
    ...
    Object.assign(projectColorForm, {
      projectId: currentProject.value.projectId as string,
      colorCode: '',
      materialCode: '',
      isPrimary: false,
      sortOrder: currentProjectColors.value.length,
      notes: '',
    })
  ...
  return {
    ...
    handleSavePalette,
    batchDeletePalettes,
    paletteItemDialogVisible,
    currentPalette,
    currentPaletteItems,
    paletteItemFormVisible,
    paletteItemForm,
    openPaletteItemDialog,
    openAddPaletteItem,
    handleSavePaletteItem,
    handleDeletePaletteItem,
    projectList,
    projectLoading,
    projectTotal,
    projectPage,
    ...
    handleSaveProject,
    batchDeleteProjects,
    projectColorDialogVisible,
    currentProject,
    currentProjectColors,
    projectColorFormVisible,
    projectColorForm,
    openProjectColorDialog,
    openAddProjectColor,
    handleSaveProjectColor,
    handleDeleteProjectColor,
  }
}
