  ...
  <div class="agent-chat">
    ...
    <div class="chat-main">
      <AgentChatSidebar
        ...
        :task-id="taskId"
        :history-tasks="
          historyTasks as unknown as Array<{
            id: string
            title: string
            status: string
            createdAt: string
      ...
      <div class="chat-mid">
          ...
          <div v-for="(msg, i) in messages" :key="i">
            <div v-if="msg.role === 'human'" class="chat-bubble human">
              <div class="bubble-meta">
                <svg class="bubble-avatar" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  <use href="/lucide-sprite.svg#user" /></svg
            ...
            <div v-else-if="msg.role === 'agent'" class="chat-bubble agent">
              <div class="bubble-meta">
                <svg
                  class="bubble-avatar agent-avatar"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="2"
                >
                  <use href="/lucide-sprite.svg#bot" /></svg
                ><span>Agent</span><span>{{ msg.time }}</span>
              </div>
                    ...
                    <div v-if="item._expanded" class="tool-card-body">
                      ...
                      <div v-if="item.progress" class="tool-progress">
                        ...
                        <span class="tool-progress-text"
                          >{{ item.progress.current }}/{{ item.progress.total }} {{ item.progress.message }}</span
                        >
                      </div>
                      <div v-if="item.streamLines && item.streamLines.length > 0" class="tool-stream">
                        <div
                          v-for="(line, li) in item.streamLines.slice(-50)"
                          :key="'sl' + li"
                          class="tool-stream-line"
                        >
                          {{ line }}
                        </div>
                      </div>
              ...
              <div v-if="msg.streaming" class="bubble-text streaming">
        ...
        <!-- 底部输入 -->
        <div v-if="!taskDone" class="chat-input-bar">
          <el-input
            v-model="inputText"
            type="textarea"
            :rows="2"
...
const { messages, renderMarkdown, formatTime, formatFooterTime, saveReActCache, saveCache, scrollBottom, triggerRef } =
  chatMessages
...
const chat = useAgentChat(
  ...
  formatFooterTime,
  syncProposals,
  insertSummary as unknown as (t: unknown, fileUrl: string, fileName: string) => void,
)
const {
  taskId,
  taskTitle,
...
.bubble-avatar.agent-avatar {
  color: #534ab7;
}
.bubble-meta span:first-child {
  font-weight: 600;
  color: #606266;
}
.bubble-text {
