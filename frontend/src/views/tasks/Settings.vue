    ...
    <el-tabs v-model="activeTab" type="border-card" class="settings-tabs">
      ...
      <el-tab-pane label="网络代理" name="network">
        <el-form label-width="140px" size="small">
          ...
          />
          <el-form-item label="HTTP 代理"><el-input v-model="s.network.httpProxy" placeholder="http://proxy.example.com:8080" /></el-form-item>
          <el-form-item label="HTTPS 代理"><el-input v-model="s.network.httpsProxy" placeholder="https://proxy.example.com:8443" /></el-form-item>
          <el-divider />
      ...
      <el-tab-pane label="API Key" name="apikey">
        <div style="min-height:360px">
          <ModelConfigTab ref="modelConfigRef" @modelsUpdated="fetchModels" />
        </div>
      </el-tab-pane>
...
async function loadAbout() {
  try {
    ...
    about.version = '1.4.0-alpha9'; about.deployMode = 'operator'
  }
  try {
...
<style scoped>
.settings-page { height: 100%; display: flex; flex-direction: column; padding: 0; }
.settings-header { display: flex; justify-content: space-between; align-items: center; padding: 12px 20px; border-bottom: 1px solid #e4e7ed; background: #fff; flex-shrink: 0; }
.settings-header h3 { margin: 0; font-size: 16px; font-weight: 700; }
.settings-tabs { flex: 1; overflow: auto; margin: 12px 16px; }
.settings-tabs :deep(.el-tabs__content) { padding: 16px 20px; min-height: 400px; }
.form-tip { font-size: 12px; color: #909399; margin-left: 10px; }
.form-tip.warning { color: #e6a23c; }
.settings-footer { padding: 8px 20px; text-align: center; border-top: 1px solid #e4e7ed; background: #fff; flex-shrink: 0; }
.save-hint { font-size: 11px; color: #c0c4cc; }
</style>
