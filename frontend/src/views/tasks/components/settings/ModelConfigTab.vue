  ...
  <div>
        ...
        <template #default="{ row }">
          <template v-if="!row.editing">
            <el-button
              ...
              text
              type="primary"
              :title="row.hasKey ? '修改 API Key' : '配置 API Key'"
    ...
    <div style="margin-top: 12px">
      <el-button size="small" @click="showAddProvider = true"
        ><el-icon><Plus /></el-icon> 添加 Provider</el-button
      >
    </div>
    <el-dialog v-model="showAddProvider" title="添加 Provider" width="480px" :close-on-click-modal="false">
      ...
      <template #footer>
        ...
        <el-button size="small" type="primary" :loading="addingProvider" @click="doAddProvider">添加</el-button>
      </template>
    </el-dialog>
    ...
    <el-form label-width="100px" size="small">
      <el-form-item label="License Key">
        <el-input
          v-model="apiKey.obaKey"
          placeholder="OBA-XXXX-XXXX-XXXX"
          style="width: 320px"
          @blur="obaKeyFormatted"
        />
      </el-form-item>
      <el-form-item>
        <el-button size="small" type="primary" :loading="activatingOba" @click="activateObaKey">验证并激活</el-button>
        <span v-if="obaStatus === 'active'" style="color: #67c23a; margin-left: 8px; font-size: 12px"
          >✅ 已激活 · 中转版</span
        >
        <span v-if="obaStatus === 'error'" style="color: #e6a23c; margin-left: 8px; font-size: 12px">{{
          obaError
        }}</span>
      </el-form-item>
      <template v-if="obaStatus === 'active'">
        <el-form-item label="Token 配额"
          ><span style="font-size: 12px; color: #606266">{{ obaQuota }}</span></el-form-item
        >
        <el-form-item label="席位"
          ><span style="font-size: 12px; color: #606266">{{ obaSeats }}</span></el-form-item
        >
      </template>
    </el-form>
  </div>
  ...
  try {
    ...
    for (const p of providers) {
      ...
      for (const m of (p.models as unknown as Array<Record<string, unknown>>) || []) {
        ...
        rows.push({
          keyId: (keyInfo?.id as string) || '',
          registryId: (m.id || '') as string,
          providerCode: (p.providerCode as string) || '',
          provider: (p.providerName as string) || (p.name as string) || '',
          model: (m.modelName as string) || (m.name as string) || '',
          modelCode: (m.modelCode as string) || (m.id as string) || '',
          baseUrl: (p.baseUrl as string) || '',
          maskedKey: hasKey ? '●●●●****' : '',
          hasKey,
          isDefault: !!isDefault,
          isBuiltin: (p.isBuiltin as boolean | null) ?? null,
          editing: false,
          editKey: '',
          saving: false,
        })
...
async function doSetDefault(row: KeyRow) {
  try {
    const res = (await request.post('/system/llm/config/set-default', {
      provider: row.providerCode,
      modelCode: row.modelCode,
    })) as unknown as Record<string, unknown>
    if (res?.success === false) {
...
async function activateObaKey() {
  ...
  try {
    ...
    obaError.value = errResp || '激活失败，请检查 Key 是否正确'
  } finally {
