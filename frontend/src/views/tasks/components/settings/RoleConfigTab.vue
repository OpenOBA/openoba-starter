    ...
    <el-tab-pane label="Agent 记忆" name="memory">
      <el-form label-width="180px" size="small">
        ...
        <el-form-item label="认知日志保留天数"
          ><el-input-number v-model="s.agent.logRetentionDays" :min="7" :max="365" /><span class="form-tip"
            >超过后归档到文件系统</span
          ></el-form-item
        >
        <el-form-item label="到期自动归档"><el-switch v-model="s.agent.autoArchiveLogs" /></el-form-item>
        <el-form-item label="Agent 自动提炼长期记忆"
          ><el-switch v-model="s.agent.autoRefineMemory" /><span class="form-tip"
            >从认知日志中自动提炼关键洞察</span
          ></el-form-item
        >
        <el-divider />
    ...
    <el-tab-pane label="Agent 行为" name="behavior">
      <el-form label-width="160px" size="small">
        ...
        <el-form-item label="最大重试次数"
          ><el-input-number v-model="s.agent.maxRetries" :min="0" :max="10" /><span class="form-tip"
            >Agent 异常自处理的重试上限</span
          ></el-form-item
        >
        ...
        <el-form-item label="自动同意方案"
          ><el-switch v-model="s.agent.autoApprove" /><span class="form-tip warning"
            >开启后跳过人工确认，直接执行（不推荐）</span
          ></el-form-item
        >
        <el-divider />
...
const props = defineProps<{
  s: {
    ...
    notifications: { enabled?: boolean; channels?: string[]; taskComplete?: boolean; errorAlert?: boolean }
    [key: string]: unknown
  }
...
async function fetchModels() {
  ...
  try {
    const res = (await request.get('/system/llm/providers')) as unknown as {
      success?: boolean
      providers?: Array<{ hasKey?: boolean; models?: Array<{ id: string; name: string; modelCode?: string }> }>
    }
    if (res?.success) {
      ...
      for (const p of res.providers ?? []) {
        ...
        for (const m of p.models ?? []) {
          allModels.push({
            id: (m.id || m.modelCode || '') as string,
            name:
              (((p as unknown as Record<string, unknown>).providerName as string) ||
                ((p as unknown as Record<string, unknown>).name as string)) +
              ' · ' +
              (((m as unknown as Record<string, unknown>).modelName as string) || m.name),
          })
        }
      }
