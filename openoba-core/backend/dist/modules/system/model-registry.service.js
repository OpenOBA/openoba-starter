...
let ModelRegistryService = ModelRegistryService_1 = class ModelRegistryService {
    ...
    async unsetDefaultModel(providerCode) {
        // 全 provider 下取消所有默认
        await this.registryRepo.update({ providerCode }, { isDefault: 0 });
        // 同步清理 keyModels
        const keys = await this.keyRepo.find({ where: { providerCode, isEnabled: 1 } });
        for (const key of keys) {
            await this.keyModelsRepo.update({ keyId: key.id }, { isDefault: 0 });
        }
    }
    async deleteKey(id) {
