...
export class AfterSales {
  ...
  @Column({ comment: 'afterSales 类型',  name: 'after_sales_type', type: 'enum', enum: ['return', 'exchange', 'refund_only', 'repair'] })
  ...
  @Column({ comment: '实际退款金额',  name: 'actual_refund_amount', type: 'decimal', precision: 10, scale: 2, nullable: true })
  ...
  /** @see APPLICANT_TYPE */
  @Column({ comment: 'applicant 类型',  name: 'applicant_type', type: 'enum', enum: ['customer', 'admin'], default: 'customer' })
