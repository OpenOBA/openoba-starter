...
export class StructureService {
  ...
  async findAll(query: QueryStructureDto) {
    ...
    if (keyword)
      qb.andWhere('(s.external_code LIKE :kw OR s.internal_code LIKE :kw OR s.description LIKE :kw)', {
        kw: `%${keyword}%`,
      })
  ...
  async update(id: string, dto: UpdateStructureStandardDto) {
    ...
    if (
      ...
      dto.width != null ||
      dto.height != null ||
      dto.circumference != null ||
      dto.bridgeWidth != null ||
      (dto as Record<string, unknown>).baseCurve != null
    ) {
      throw new BadRequestException(
        '物理属性不支持编辑修改。如需变更物理规格，请废弃当前标准（status=deprecated）后新建。',
      )
    }
  ...
  async addAttachment(dto: CreateAttachmentDto) {
    const entity = this.attachRepo.create({
      ...dto,
      attachmentId: `att-${Date.now()}-${crypto.randomUUID().replace(/-/g, '').substring(0, 8)}`,
    })
