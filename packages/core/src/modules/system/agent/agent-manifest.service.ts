...
export class AgentManifestService {
  ...
  /** 更新 Agent 信息（名称/标识/安全等级/状态/关联用户） */
  async update(agentId: string, dto: {
    agentCode?: string
    agentName?: string
    securityClearance?: string
    status?: string
    ...
    if (dto.agentCode !== undefined) agent.agentCode = dto.agentCode
    if (dto.agentName !== undefined) agent.agentName = dto.agentName
    if (dto.securityClearance !== undefined) agent.securityClearance = dto.securityClearance
    if (dto.status !== undefined) agent.status = dto.status
    if (dto.userId !== undefined) agent.userId = dto.userId
